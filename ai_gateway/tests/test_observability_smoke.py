"""Ensure the AI Gateway exposes metrics and traces."""
...
def test_metrics_and_traces_exposed(client) -> None:
    ...
    request_sample = next(
        ...
        if sample.labels["service"] == "rbok-ai-gateway"
    ...
    assert any(
        sample.labels["route"] == "/healthz" and sample.labels["service"] == "rbok-ai-gateway"
    ...
    gauge_sample = next(
        ...
        and sample.labels["service"] == "rbok-ai-gateway"
        and sample.labels["method"] == "GET"
    )
